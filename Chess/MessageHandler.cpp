...
std::string MessageHandler::encode(int send_x_index, int send_y_index, int send_toPixel_X, int send_toPixel_y)
{
	std::stringstream msg_tmp;
	msg_tmp << send_x_index << "-" << send_y_index << "-" << send_toPixel_X << "-" << send_toPixel_y<<"-";
	return msg_tmp.str();
}
